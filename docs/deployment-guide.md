...
**Chú ý:** Dashboard sẽ được HTTPProxy của `htn-tunnel` tự động reverse-proxy nội bộ từ HTTPS (port 8443) sang HTTP (port 1807). Do đó, Nginx chỉ cần pass toàn bộ traffic `*.33.id.vn` vào port 8443.
